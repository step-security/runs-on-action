...
func prepareBuildkitVolume(action *githubactions.Action, stateRoot string) error {
	...
	if found {
		if !dockerVolumeOwnedByRunsOn(volume) {
			return fmt.Errorf("Docker volume %s already exists but is not owned by RunsOn; run step-security/runs-on-action before docker/setup-buildx-action", buildkitStateVolumeName)
		}
...
func inspectBuildkitContainer() (dockerContainerInspect, error) {
	...
	if err != nil {
		return dockerContainerInspect{}, fmt.Errorf("inspect Buildx container %s: %w: %s; docker/setup-buildx-action must run after step-security/runs-on-action with cleanup=false", buildkitContainerName, err, strings.TrimSpace(string(out)))
	}
