	...
	for i, target := range canonical {
		for j, mountedTarget := range mountedTargets {
			...
			if pathContains(target, mountedTarget) || pathContains(mountedTarget, target) {
				return fmt.Errorf("sticky cache path %s overlaps %s mounted by an earlier step-security/runs-on-action invocation", targets[i], mountedTargets[j])
			}
		}
	}
	return nil
...
func validateCacheOrdering(requests []CacheRequest, goos, home, workspace string) error {
	...
	if len(workspaceModes) > 0 {
		return fmt.Errorf("git cache mode cannot be combined with workspace-relative cache modes (%s) in one invocation; run git before checkout, then run workspace-relative caches in a second step-security/runs-on-action step after checkout", strings.Join(workspaceModes, ", "))
	}
	return nil
}
