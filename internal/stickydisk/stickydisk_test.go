...
func TestValidateCacheOrdering(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr && (err == nil || !strings.Contains(err.Error(), "separate step-security/runs-on-action steps") && !strings.Contains(err.Error(), "second step-security/runs-on-action step")) {
...
func TestValidateNoOverlappingTargetsIncludesEarlierInvocations(t *testing.T) {
	...
	if err := validateNoOverlappingTargetsWithMounted([]string{"/workspace/vendor/cache"}, mounted, "/runs-on/stickydisk"); err == nil || !strings.Contains(err.Error(), "earlier step-security/runs-on-action invocation") {
