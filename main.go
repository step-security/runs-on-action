...
import (
	...
	"github.com/step-security/runs-on-action/internal/cache"
	"github.com/step-security/runs-on-action/internal/config"
	"github.com/step-security/runs-on-action/internal/costs"
	"github.com/step-security/runs-on-action/internal/env"
	"github.com/step-security/runs-on-action/internal/gitproxy"
	"github.com/step-security/runs-on-action/internal/monitoring"
	"github.com/step-security/runs-on-action/internal/sccache"
	"github.com/step-security/runs-on-action/internal/stickydisk"
)
...
func handleMainExecution(action *githubactions.Action, ctx context.Context) {
	...
	if claimCostTracking(action, cfg.ShowCosts) {
		...
		action.Infof("Cost tracking is already registered by an earlier step-security/runs-on-action invocation; this invocation will skip duplicate cost reporting.")
	}
...
func handlePostExecution(action *githubactions.Action, ctx context.Context) {
	...
	if shouldTrackCostsInPost() {
		...
		action.Infof("Skipping duplicate cost reporting from this step-security/runs-on-action invocation.")
	} else {
		action.Infof("Cost reporting is disabled for this step-security/runs-on-action invocation.")
	}
